              ...
              {filteredItems.length === 0 && query !== "" ? (
                ...
                filteredItems.map((utility) => (
                  <Combobox.Option
                    key={utility.id}
                    className={({ active }) =>
                      `relative cursor-pointer select-none py-2 pl-10 pr-4 transition duration-300 ${
                  ...
                  >
                    {({ active }) => (
                      <Link href={utility.link}>
                        <span className="">{utility.name}</span>
                        <span
                          className={`absolute inset-y-0 left-0 flex items-center pl-3 transition duration-300 text-lg ${
